...
public class PaymentController {
    ...
    /**
     * @Value("${server.port}") Spring 注解，获取 yml 配置文件中的端口号
     */
    @Value("${server.port}")
    private String serverPort;

    // springframework 的 DiscoveryClient，不要导错包了
    @Resource
    private DiscoveryClient discoveryClient;

    /**
     * 前后端分离，所以不能直接返回对象，数据要先经过 CommonResult 封装再返回
     * 注意这里的 @RequestBody 是必须要写的，虽然 MVC 可以自动封装参数成为对象
     * 是当消费者项目调用，它传参是 payment，整个实例对象传过来的，即 Json 数据，因此需要写这个注解
     * 如果不写 @RequestBody，当消费者调用此服务进行创建时，虽然可以创建成功，单数据库中只有主键，没有数据
     * @param payment
     * @return
     */
